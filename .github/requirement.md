...
We have provided an Angular (our framework of choice) boilerplate for you to fork: [https://github.com/kin/js-challenge-boilerplate](https://github.com/kin/js-challenge-boilerplate)[cite: 1].

- **Frameworks**: Please refrain from using a different javascript (e.g., React or Vue) or css framework (e.g., Material UI or Bootstrap); other minor dependencies are fine.
- **Language**: We are a Typescript-first shop and value your understanding of TS/JS over using specific Angular patterns. Your submission should showcase your capabilities. Be creative and play to your strengths.
...
1. **Invite Collaborators**: Invite `@kinrobot`, `@tgrux`, `@gecugamo`, `@KevinGreene` and `@bdivkin` to a private GitHub repo and set them as collaborators.
2. **Forking**: Once your submission is complete, we will fork it. No changes will be accepted after the fork, so save it for discussion if there's a last-minute change.
3. **README**: Please include instructions for installing, running, and testing your code in a `README.md` file at the root of your project.
4. **Final Submission**: Additionally, per the instructions in the email, we ask you to submit a PDF that simply lists the following:
   - Your Name
   - Your GitHub username
   - A link to your private GitHub repository
5. **Privacy**: Please do not open your repo to the public or share your solution.
...
| Senior, Staff, Sr. Staff, or Principal | Stories 1-3                     |

### User Story 1: Digit Parser
...
### User Story 3: UI Implementation

Your boss is excited when they see your new application and ask you to send the processed policy numbers to our API. Your application should post the array of policy objects (from user story 2) to https://jsonplaceholder.typicode.com/posts (this is a mock API endpoint, documentation here); then, it should show the user a success or failure message, including the ID from the endpoint response.

### User Story 4: Correction Logic
...
Your last task is to check if an invalid policy number could be valid by swapping a number that could have been mixed up. If there is only one possible number with a valid checksum, then use that. If there are several options, the status should be AMB. If you still can't work out what it should be, the status should be reported as an error.
